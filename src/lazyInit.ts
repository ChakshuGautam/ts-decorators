...
export const LazyInit = (
  ...
  if (initializer === undefined) {
    throw `@LazyInit can't be set as a property \`${name}\` on ${constructor.name} class, using a getter instead!`
  }
  if (setter) {
    throw `@LazyInit can't be annotated with get ${name}() existing a setter on ${constructor.name} class!`
  }
